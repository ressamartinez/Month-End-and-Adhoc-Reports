...
SELECT *,
       CASE WHEN tempc.[Overdue in Days] <= 0 then 'Current'
			...
			WHEN tempc.[Overdue in Days] >= 121 and tempc.[Overdue in Days] <= 151 then '121 to 150'
			WHEN tempc.[Overdue in Days] >= 152 and tempc.[Overdue in Days] <= 182 then '151 to 180'
			WHEN tempc.[Overdue in Days] >= 183 and tempc.[Overdue in Days] <= 213 then '181 to 210'
			WHEN tempc.[Overdue in Days] >= 214 and tempc.[Overdue in Days] <= 244 then '211 to 240'
			WHEN tempc.[Overdue in Days] >= 245 and tempc.[Overdue in Days] <= 275 then '241 to 270'
			WHEN tempc.[Overdue in Days] >= 276 and tempc.[Overdue in Days] <= 306 then '271 to 300'
			WHEN tempc.[Overdue in Days] >= 307 and tempc.[Overdue in Days] <= 337 then '301 to 330'
			WHEN tempc.[Overdue in Days] >= 338 and tempc.[Overdue in Days] <= 360 then '331 to 360'
			WHEN tempc.[Overdue in Days] > 360 then '>360'
	        end as Aging
from (
