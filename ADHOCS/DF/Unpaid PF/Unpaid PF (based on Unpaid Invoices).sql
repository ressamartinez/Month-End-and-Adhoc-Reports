...
SET @dTo = /*@To*/ '12/31/2018 23:59:59.998'
set @AsOFDate2 = /*@AsOf*/ '12/31/2018  23:59:59.998'	--27874


select distinct 
tempc.[Employee NR],
				tempc.[Caregiver Name],
				tempc.HN,
				tempc.[Patient Name],
				tempc.[Visit Type],
				tempc.item_code as [Item Code],
				tempc.name_l as [Item Description],
				tempc.gross_amount as [Gross Amount],
				tempc.[Net Amount],
				tempc.discount_amount as [Discount Amount],
				tempc.[Invoice No.],
				tempc.[Transaction Date and Time]
				--tempc.[Readers Fee],
				--tempc.S23,
				--[Total Unpaid] = tempc.[Readers Fee] + tempc.S23
...
SELECT distinct tempb.[Invoice No.],
		   ...
		   --tempb.[Net HB],
		   --tempb.[Discount HB],
		   tempb.[Readers Fee Net],
		   --tempb.[Readers Fee Discount],
		   tempb.[Net PF S23],
		   --tempb.[Discount PF S23],
		   tempb.[Net ER PF],
		   [Readers Fee] = tempb.[Readers Fee Net] + tempb.[Net ER PF],
		   S23 = tempb.[Philhealth PF] + tempb.[Net PF S23]
		   --tempb.[Discount ER PF],
		   --tempb.[Package Discount],
		   --tempb.[Payment Status],
		   --tempb.[Owing Amount],
		   --[Overdue in Days] = DATEDIFF(day,tempb.[Transaction Date and Time],'12/31/2018'),
		   --tempb.[Due Date],
		   --[Due Date in Days] = DATEDIFF(day,tempb.[Due Date],'12/31/2018')
		   ,invoice_id
		   ,cc.employee_nr as [Employee NR]
		   ,RTRIM(cc.caregiver_lname) + ', ' + RTRIM(cc.caregiver_fname) + ' ' + RTRIM(cc.caregiver_mname) as [Caregiver Name]
		   ,i.item_code
		   ,i.name_l
		   ,ard.gross_amount
		   ,ard.discount_amount
		   ,[Net Amount] = ard.gross_amount - ard.discount_amount
...
inner join AmalgaPROD.dbo.ar_invoice_detail ard on ard.ar_invoice_id = tempb.invoice_id
inner join AmalgaPROD.dbo.item i on i.item_id = ard.item_id
inner join itworksdev01.dis_test_2017.dbo.charge_caregiver cc on cc.charge_id = ard.charge_detail_id
inner join itworksdev01.dis_test_2017.dbo.charge_detail cd on cd.charge_id = cc.charge_id
where tempb.[Invoice No.] not in ('PINV-2006-003407',
	...
	--and tempb.[Invoice No.] = 'PINV-2018-353276'



	)as tempc
	--where [Employee NR] is not null
		 
order by tempc.[Transaction Date and Time],
		 tempc.[Invoice No.],
		 tempc.[Patient Name]