...
--unpaid
SELECT dba.employee_nr as [Employee No],
	   ...
	   aih.transaction_date_time as [Transaction Date]

from df_browse_all dba left outer join ar_invoice_detail_head aidh on aidh.charge_detail_id = dba.charge_id
					   left outer join ar_invoice_detail_tail aidt on aidt.ar_invoice_detail_id = aidh.ar_invoice_detail_id
					   left outer join ar_invoice_head aih on aih.ar_invoice_id = aidh.ar_invoice_id
					   left outer join ar_invoice_tail ait on ait.ar_invoice_id = aih.ar_invoice_id
...
where ait.transaction_status_rcd not in ('unk','voi')
	...
	-- and ard.charge_detail_id = '931FEA08-72D8-11E8-A2C9-9E78655E10C3'
	 ...
	 and CAST(CONVERT(VARCHAR(10),aih.transaction_date_time,101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),'12/31/2018',101) as SMALLDATETIME)
	 --and aih.transaction_text = 'PINV-2018-353367'
	 --and dba.item_code = '210-15-0106'
