SELECT aih.transaction_date_time as [Transaction Date],
	   ...
	   RTRIM(dba.caregiver_lname) + ', ' + RTRIM(dba.caregiver_fname)
	    as [Doctor Name],
	   dba.upi as [HN No],
...
from df_browse_all dba inner join ar_invoice_detail_head aidh on aidh.charge_detail_id = dba.charge_id
					   inner join ar_invoice_detail_tail aidt on aidt.ar_invoice_detail_id = aidh.ar_invoice_detail_id
					   inner join ar_invoice_head aih on aih.ar_invoice_id = aidh.ar_invoice_id
					   inner join ar_invoice_tail ait on ait.ar_invoice_id = aih.ar_invoice_id
					   left outer join payment_period_detail_history ppdh on ppdh.ar_invoice_detail_id = aidh.ar_invoice_detail_id
					   inner join gl_acct_code gac on gac.gl_acct_code_id = aidt.gl_acct_code_Credit_id
...
where ait.transaction_status_rcd not in ('unk','voi')
	...
	-- and ard.charge_detail_id = '931FEA08-72D8-11E8-A2C9-9E78655E10C3'
	 ...
	 and CAST(CONVERT(VARCHAR(10),aih.transaction_date_time,101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),'09/01/2018',101) as SMALLDATETIME)
	 and CAST(CONVERT(VARCHAR(10),aih.transaction_date_time,101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),'09/30/2018',101) as SMALLDATETIME)
