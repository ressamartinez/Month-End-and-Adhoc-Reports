...
SET @dFrom = /*@From*/ '01/01/2017 00:00:00.000'		--Jan 2018 = 22667
SET @dTo = /*@To*/ '12/31/2017 23:59:59.998'
set @AsOFDate2 = /*@AsOf*/ '12/31/2017  23:59:59.998'	--27745


SELECT *,
       CASE --WHEN tempc.[Overdue in Days] >= -30 and tempc.[Overdue in Days] <= 0 then 'Current'
	        WHEN tempc.[Overdue in Days] >= 0 and tempc.[Overdue in Days] <= 30 then '0-30'
			WHEN tempc.[Overdue in Days] >= 31 and tempc.[Overdue in Days] <= 60 then '31-60'
			WHEN tempc.[Overdue in Days] >= 61 and tempc.[Overdue in Days] <= 90 then '61-90'
			WHEN tempc.[Overdue in Days] >= 91 and tempc.[Overdue in Days] <= 120 then '91-120'
			WHEN tempc.[Overdue in Days] >= 121 and tempc.[Overdue in Days] <= 151 then '121-151'
			WHEN tempc.[Overdue in Days] >= 152 and tempc.[Overdue in Days] <= 182 then '152-182'
			WHEN tempc.[Overdue in Days] >= 183 and tempc.[Overdue in Days] <= 213 then '183-213'
			WHEN tempc.[Overdue in Days] >= 214 and tempc.[Overdue in Days] <= 244 then '214-244'
			WHEN tempc.[Overdue in Days] >= 245 and tempc.[Overdue in Days] <= 275 then '245-275'
			WHEN tempc.[Overdue in Days] >= 276 and tempc.[Overdue in Days] <= 306 then '276-306'
			WHEN tempc.[Overdue in Days] >= 307 and tempc.[Overdue in Days] <= 337 then '307-337'
			WHEN tempc.[Overdue in Days] >= 338 and tempc.[Overdue in Days] <= 365 then '338-365'
			WHEN tempc.[Overdue in Days] >= 366 and tempc.[Overdue in Days] <= 999 then '366-999'
			WHEN tempc.[Overdue in Days] > 999 then '>999'
	        end as Aging
from (
...
SELECT distinct tempb.[Invoice No.],
	   ...
	   --tempb.[Payor],
		  ...
		  tempb.[Transaction Date and Time] as 'Invoice Date',
		   ...
		   [Overdue in Days] = DATEDIFF(day, tempb.[Transaction Date and Time],'12/31/2017')
from
(
	SELECT temp.hn as [HN],
		   ...
		   temp.net_df as [Readers Fee Net],
		   temp.discount_df as [Readers Fee Discount],
				...
				isnull((SELECT ISNULL(SUM(temp.net_amount),0)
										...
										(
											...
											where a.ar_invoice_id =   ar_main.ar_invoice_id
												and b2.transaction_date_time <= @AsOFDate2
	...
	where CAST(CONVERT(VARCHAR(10),inv.transaction_date_time,101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),@dFrom,101) as SMALLDATETIME)
		 ...
		 and ar_main.transaction_status_rcd not in ('unk','voi')
		  and inv.invoice_id not in (SELECT a.ar_invoice_id
													from AmalgaPROD.dbo.swe_ar_instalment a INNER JOIN AmalgaPROD.dbo.remittance b on a.remittance_id  = b.remittance_id
																									inner JOIN AmalgaPROD.dbo.ar_invoice c on a.ar_invoice_id = c.ar_invoice_id
													where b.transaction_date_time <= @AsOFDate2
														and b.transaction_status_rcd <> 'voi')
...
where tempb.[Invoice No.] not in ('PINV-2006-003407',
								...
								'PINV-2017-101559',
								'PINV-2017-057248'	--with amount of 452.40 partial paid but when checked, already paid so must not be included in extraction (asof Mar-Apr 2018) (COM - 2018-01-29 11:54:57.543)
								)
