...
from (
		...
		select si.item_id
			   ...
			   ,si.store_id
			   ,s.name_l as Store
			   ,Jan = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
			   ...
			   ,Feb = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 2),0)
			   ,Mar = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 3), 0)
			   ,Apr = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 4), 0)
			   ,May = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 5), 0)
			   ,Jun = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 6), 0)
			   ,Jul = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 7), 0)
			   ,Aug = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 8), 0)
			   ,Sept = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 9), 0)
			   ,Oct = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 10), 0)
			   ,Nov = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
							and YEAR(_isd.date_time) = @Year and MONTH(_isd.date_time) = 11), 0)
			   ,Dec = isnull((select sum(issue_to_store) + sum(issue_to_store_deleted) + sum(issue_to_dept)
							from inventory_summary_day _isd
							where _isd.item_id = si.item_id
							and _isd.store_id = si.store_id
...
--where temp.item_code in ('204038047', '208058020')
