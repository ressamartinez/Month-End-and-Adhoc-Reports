...
SELECT  case when phu.visible_patient_id is not NULL then phu.visible_patient_id ELSE		
	   ...
	   ,case when phu.visible_patient_id is not NULL then (case when p.name_l is null then  pfn.display_name_l else p.name_l end) ELSE
		...
		--,case when p.name_l is null then  pfn.display_name_l else p.name_l end as [Payor Name]
		,(select employee_nr from employee_employment_info_view where person_id = cd.caregiver_employee_id) as [Employee NR]
		,case when cd.caregiver_employee_id <> '00000000-0000-0000-0000-000000000001' THEN isnull((select display_name_l from person_formatted_name_iview_nl_view where person_id = cd.caregiver_employee_id),'') end as [Doctor Name]
		
		,tsr.name_l as [Invoice Status]	
	    ,vtr.name_l as [Visit Type]
		, (SELECT gl_acct_code_code from gl_acct_code WHERE gl_acct_code_id = ard.gl_acct_code_credit_id)  as [GL Account Code]
		, (SELECT name_l from gl_acct_code WHERE gl_acct_code_id = ard.gl_acct_code_credit_id)  as [Gl Account Name]
...
from ar_invoice_nl_view ar inner JOIN gl_acct_code gac on ar.gl_acct_code_debit_id = gac.gl_acct_code_id
					   ...
					   LEFT OUTER JOIN patient_visit_nl_view pv on cd.patient_visit_id = pv.patient_visit_id
						...
						LEFT OUTER JOIN item i on ard.item_id = i.item_id
						 ...
						 inner JOIN gl_transaction glt on ar.gl_transaction_id = glt.gl_transaction_id
...
where ar.transaction_status_rcd not in ('voi','unk')
    and MONTH(ar.transaction_date_time) = 8
	and YEAR(ar.transaction_date_time) = 2018
	and gac.gl_acct_code_code = '1130400'
	and cd.deleted_date_time is NULL
    and (SELECT gl_acct_code_code from gl_acct_code WHERE gl_acct_code_id = ard.gl_acct_code_credit_id) in ('2152100','4264000')
