...
SELECT temp.monthid,
	...
	from
(
select  DISTINCT MONTH(tbl1.receive_due_date) as monthid,
		    case when MONTH(tbl1.receive_due_date) = 1 then 'January'
				 when MONTH(tbl1.receive_due_date) = 2 then 'February'
				 when MONTH(tbl1.receive_due_date) = 3 then 'March'
			     when MONTH(tbl1.receive_due_date) = 4 then 'April'
			     when MONTH(tbl1.receive_due_date) = 5 then 'May'
				 when MONTH(tbl1.receive_due_date) = 6 then 'June'
				 when MONTH(tbl1.receive_due_date) = 7 then 'July'
				 when MONTH(tbl1.receive_due_date) = 8 then 'August'
				 when MONTH(tbl1.receive_due_date) = 9 then 'September'
				 when MONTH(tbl1.receive_due_date) = 10 then 'October'
				 when MONTH(tbl1.receive_due_date) = 11 then 'November'
				 when MONTH(tbl1.receive_due_date) = 12 then 'December' 
		   END as month_name,
		  ...
		  (SELECT sum(temp.total_amt)
			...
			(
				...
				where organisation_type = tbl1.organisation_type
					and MONTH(receive_due_date) = MONTH(tbl1.receive_due_date)
		    ...
		    isnull((SELECT sum(sai_net_amount)
		   	...
		   	isnull((SELECT sum(sai_net_amount)
					...
					where sai_payment_status = 'COM'
						...
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as overdue_31_60,
			isnull((SELECT sum(sai_net_amount)
					from @table
					where sai_payment_status = 'COM'
						and aging >= 61 and aging <= 90
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as overdue_61_90,
			isnull((SELECT sum(sai_net_amount)
					from @table
					where sai_payment_status = 'COM'
						and aging >= 91 and aging <= 120
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as overdue_91_120,
			isnull((SELECT sum(sai_net_amount)
					from @table
					where sai_payment_status = 'COM'
						and aging >= 121 and aging <= 365
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as overdue_121_365,
			isnull((SELECT sum(sai_net_amount)
					from @table
					where sai_payment_status = 'COM'
						and aging >= 366 and aging <= 999
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as overdue_366_999,
		 isnull((SELECT sum(sai_net_amount)
					from @table
					where sai_payment_status = 'COM'
						and aging >  999
						and organisation_type = tbl1.organisation_type
						and MONTH(receive_due_date) =  MONTH(tbl1.receive_due_date)),0) as over_999
from @table tbl1
--where tbl1.organisation_type = 'Self Pay'
--  and MONTH(tbl1.transaction_date_time) = 1
group by  MONTH(tbl1.receive_due_date), tbl1.organisation_type
) as temp
