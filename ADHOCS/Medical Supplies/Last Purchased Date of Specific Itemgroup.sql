...
(
		select ig.name_l as item_group_name,
			   ...
			   --i.item_id,
			   --created_on_date = (Select top 1 spo.created_on_date 
						...
						--	where cast(convert(varchar(10),spo.created_on_date,101)as smalldatetime) >= cast(convert(varchar(10),'01/01/2018',101)as smalldatetime)
						--	      and cast(convert(varchar(10),spo.created_on_date,101)as smalldatetime) <= cast(convert(varchar(10),'12/31/2018',101)as smalldatetime)
						--	and svi.item_id = isd.item_id
						--	order by created_on_date desc)
			   po_date = (Select top 1 spo.ordered_on_date 
							from swe_purchase_order spo inner join swe_purchase_order_detail spod on spo.purchase_order_id = spod.purchase_order_id
														inner join swe_vendor_item svi on svi.vendor_item_id = spod.vendor_item_id
							where cast(convert(varchar(10),spo.ordered_on_date,101)as smalldatetime) >= cast(convert(varchar(10),'01/01/2018',101)as smalldatetime)
							      and cast(convert(varchar(10),spo.ordered_on_date,101)as smalldatetime) <= cast(convert(varchar(10),'12/31/2018',101)as smalldatetime)
							and svi.item_id = isd.item_id
							order by spo.ordered_on_date desc),
			   qty = (Select top 1 spod.ordered_qty 
							from swe_purchase_order spo inner join swe_purchase_order_detail spod on spo.purchase_order_id = spod.purchase_order_id
														inner join swe_vendor_item svi on svi.vendor_item_id = spod.vendor_item_id
							where cast(convert(varchar(10),spo.ordered_on_date,101)as smalldatetime) >= cast(convert(varchar(10),'01/01/2018',101)as smalldatetime)
							      and cast(convert(varchar(10),spo.ordered_on_date,101)as smalldatetime) <= cast(convert(varchar(10),'12/31/2018',101)as smalldatetime)
		...
		where i.active_flag = 1
			  ...
			  and ig.item_group_id = '9C035CF1-502B-11DA-BB34-000E0C7F3ED2'
			  --and i.item_code = '588-0018'
) as temp
--where temp.created_on_date is not null
