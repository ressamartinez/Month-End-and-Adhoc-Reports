...
from
(
select distinct dba.employee_nr,
	   dba.caregiver_lname,
       dba.caregiver_fname,
...
where dba.validated = 'Y'
	  and CAST(CONVERT(VARCHAR(10),dba.charge_date,101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),'09/01/2019',101) as SMALLDATETIME)
	  and CAST(CONVERT(VARCHAR(10),dba.charge_date,101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),'09/30/2019',101) as SMALLDATETIME)

)as temp
where temp.department <> 'Rehabilitation Medicine'
      --and temp.employee_nr = '3573'
