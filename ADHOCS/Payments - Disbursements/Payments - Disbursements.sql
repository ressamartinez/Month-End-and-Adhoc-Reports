...
select DISTINCT 
	   ...
	   apv.net_amount,
	   apv.net_amount - ap.pay_amount as unallocated_amount,
	   ap.wth_tax_amount,
	   ap.pay_amount,
...
from ap_payment ap left outer join vendor v on ap.vendor_id = v.vendor_id 
				   ...
				   left outer join gl_transaction gl on ap.gl_transaction_id = ap.gl_transaction_id
				   ...
				   left outer join ap_payment_vendor apv on ap.ap_payment_id = apv.ap_payment_id
				   left outer join gl_acct_code gac on ap.credit_gl_acct_code_id = gac.gl_acct_code_id

where CAST(CONVERT(VARCHAR(10),ap.payment_date_time,101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),'06/15/2019',101) as SMALLDATETIME)
      and CAST(CONVERT(VARCHAR(10),ap.payment_date_time,101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),'07/15/2019',101) as SMALLDATETIME)
