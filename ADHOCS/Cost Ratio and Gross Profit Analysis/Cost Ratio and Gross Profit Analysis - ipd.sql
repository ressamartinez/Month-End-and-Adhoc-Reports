...
--count(distinct tempc.patient_visit_id)
	...
	,tempc.[Visit Start]
	,tempc.[Closure Date]
	,(Select item_group_code from item_group where item_group_id = ig.parent_item_group_id) as [Main Group Code]
	,(Select name_l from item_group where item_group_id = ig.parent_item_group_id) as [Main Group Name]
	--,ig.item_group_code
	--,ig.name_l as item_group_name
	,i.item_code collate sql_latin1_general_cp1_cs_as as [Item Code]
	,i.name_l collate sql_latin1_general_cp1_cs_as as [Item Name]
	,tempc.HN
	,tempc.[Patient Name]
...
where 
      ...
      --and month(tempb.[Visit Start]) = 11
	   CAST(CONVERT(VARCHAR(10),tempb.[Visit Start],101) as SMALLDATETIME) >= CAST(CONVERT(VARCHAR(10),'01/01/2020',101) as SMALLDATETIME)
	  and CAST(CONVERT(VARCHAR(10),tempb.[Visit Start],101) as SMALLDATETIME) <= CAST(CONVERT(VARCHAR(10),'01/15/2020',101) as SMALLDATETIME)
...
where ar.transaction_status_rcd not in ('voi', 'unknown')

order by tempc.HN, [Item Name], [Invoice Number]
